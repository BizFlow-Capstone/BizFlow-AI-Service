...
class DraftCostResult(BaseModel):
    ...
    cost_type: str | None = None      # VD: "Tiện ích", "Nguyên liệu", "Nhân công", "Vận chuyển", "Khác"
...
Nhiệm vụ: từ câu nói của chủ shop, trích xuất thông tin chi phí thành JSON.
...
- "cost_type": phân loại chi phí dựa trên nội dung:
    "Tiện ích"   → điện, nước, internet, điện thoại
    "Nguyên liệu" → nguyên liệu, vật tư, hàng hóa mua vào
    "Nhân công"  → lương, thưởng, công thợ
    "Vận chuyển" → ship, giao hàng, vận chuyển
    "Thuê mặt bằng" → thuê nhà, thuê mặt bằng
    "Khác"       → nếu không thuộc các loại trên
- "confidence": "high" nếu trích xuất đủ amount + description; "medium" nếu chỉ rõ một trong hai; "low" nếu quá mơ hồ.
...
async def process_draft_cost(
    ...
    if not transcript.strip():
        return DraftCostResult(raw_transcript="", confidence="low")

    # Step 2: LLM extraction
    user_prompt = (
        f"Câu nói của chủ shop: \"{transcript}\"\n\n"
        "Trích xuất thông tin chi phí theo format JSON đã quy định."
    )
...
def _parse_llm_response(raw: str, transcript: str) -> DraftCostResult:
    try:
        ...
        return DraftCostResult(raw_transcript=transcript, confidence="low")
